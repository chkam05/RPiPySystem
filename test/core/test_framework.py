...
def testcase(fn: Callable) -> Callable:
    """Marks a method as a runnable test case."""
...
class TestCase:
    """Small live-test framework inspired by C# unit-test assertions."""

    def config(self) -> None:
        """Runs once before all @testcase methods. Override in child classes."""

    def run(self, script_name: Optional[str] = None) -> bool:
        """Runs config() first, then all @testcase methods ordered by name."""
        script_name = script_name or (sys.argv[0].split('/')[-1] or self.__class__.__name__).strip()
        results: list[TestResult] = []

        try:
            self.config()
        except Exception as ex:
            print(f'[{script_name}] config: False "{ex}"')
            raise
        ...
        for name, method in methods:
            ...
            try:
                method()
                result = TestResult(name, True, time.perf_counter() - started)
            except AssertionError as ex:
                result = TestResult(name, False, time.perf_counter() - started, str(ex))
            except Exception as ex:
                result = TestResult(name, False, time.perf_counter() - started, str(ex))

            results.append(result)
            error = f' "{result.error}"' if result.error else ''
            print(f'[{script_name}] {name}: {result.passed} ({result.elapsed_seconds:.3f}s){error}')

        passed = sum(1 for result in results if result.passed)
        print(f'[{script_name}] summary: {passed}/{len(results)} passed')
    ...
    def assert_false(self, condition: bool, message: Optional[str] = None) -> None:
        if condition is not False:
            self._fail(message or 'Expected condition to be False.')
    ...
    def are_not_equal(self, value: Any, unexpected_value: Any, message: Optional[str] = None) -> None:
        if value == unexpected_value:
            self._fail(message or f'Expected value different than {unexpected_value!r}.')
    ...
    def ends_with(self, value: Any, expected_ending: Any, message: Optional[str] = None) -> None:
        ...
        if isinstance(value, str) and isinstance(expected_ending, str):
            ...
            try:
                ok = list(value)[-len(expected_ending):] == list(expected_ending)
            except Exception:
                ok = False
        if not ok:
    ...
    def has_count(self, enumerable_value: Iterable, expected_count: int, message: Optional[str] = None) -> None:
        try:
            count = len(enumerable_value)  # type: ignore[arg-type]
        except Exception:
            count = sum(1 for _ in enumerable_value)
        if count != expected_count:
            self._fail(message or f'Expected count {expected_count}, got {count}.')
    ...
    def is_not_empty(self, value: Any, message: Optional[str] = None) -> None:
        try:
            ...
            ok = bool(value)
        if not ok:
            self._fail(message or 'Expected non-empty value.')
    ...
    def is_type(self, value: Any, expected_type: Type, message: Optional[str] = None) -> None:
        if type(value) is not expected_type:
            self._fail(message or f'Expected exact type {expected_type}, got {type(value)}.')

    def is_not_type(self, value: Any, unexpected_type: Type, message: Optional[str] = None) -> None:
        if type(value) is unexpected_type:
            self._fail(message or f'Expected type different than {unexpected_type}.')

    def starts_with(self, value: Any, expected_start: Any, message: Optional[str] = None) -> None:
        ok = False
        if isinstance(value, str) and isinstance(expected_start, str):
            ok = value.startswith(expected_start)
        else:
            try:
                ok = list(value)[:len(expected_start)] == list(expected_start)
            except Exception:
                ok = False
        if not ok:
            self._fail(message or f'Expected {value!r} to start with {expected_start!r}.')

    # Backward-compatible alias for the typo used in older tests.
    def start_with(self, value: Any, expected_start: Any, message: Optional[str] = None) -> None:
        self.starts_with(value, expected_start, message)
