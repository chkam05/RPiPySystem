...
class ProcessInfo(PublicModel):
    ...
    _DATE_TIME_FORMATS: ClassVar[Tuple[str]] = ('%Y-%m-%d %H:%M:%S', '%Y.%m.%d-%H:%M:%S')
    ...
    @classmethod
    def _parse_datetime_str(cls, v: Any) -> Optional[datetime]:
        ...
        for fmt in cls._DATE_TIME_FORMATS:
            try:
    ...
    def _fmt_datetime(self, dt: Optional[datetime]) -> Optional[str]:
        return dt.strftime(self._DATE_TIME_FORMATS[0]) if isinstance(dt, datetime) else None
    ...
    @classmethod
    def from_dict(cls: Type[T], d: Dict[str, Any]) -> ProcessInfo:
        return cls(
