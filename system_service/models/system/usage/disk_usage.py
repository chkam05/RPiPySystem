...
T = TypeVar('T', bound='DiskUsage')


@dataclass
class DiskUsage(PublicModel):
    ...
    @classmethod
    def from_dict(cls: Type[T], d: Dict[str, Any]) -> DiskUsage:
        fs_str = d.get(cls.FIELD_FS_TYPE)
        fs_val: DiskType.OTHER

        if fs_str:
            try:
                fs_val = DiskType(fs_str) if isinstance(fs_str, str) else DiskType.OTHER
