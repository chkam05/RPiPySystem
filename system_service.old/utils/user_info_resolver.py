...
class UserInfoResolver:
    ...
    @classmethod
    def get_logged_in_users(cls) -> List[UserLoggedIn]:
        """
        ...
        for wr in who_rows:
            ...
            payload: Dict[str, Any] = {
                UserLoggedIn.FIELD_USER_NAME: wr.get('login'),
                UserLoggedIn.FIELD_TERMINAL_NAME: wr.get('term'),
                UserLoggedIn.FIELD_LOGGED_AT: wr.get('time'),
                UserLoggedIn.FIELD_REMOTE_HOST: wr_w.get('from') if wr_w else None,
                UserLoggedIn.FIELD_IDLE_TIME: wr.get('idle_or_time'),
                UserLoggedIn.FIELD_JOB_CPU_TIME: wr_w.get('jcpu') if wr_w else None,
                UserLoggedIn.FIELD_PROCESS_CPU_TIME: wr_w.get('pcpu') if wr_w else None,
                UserLoggedIn.FIELD_SESSION_COMMAND: wr_w.get('what') if wr_w else None,
                UserLoggedIn.FIELD_PROCESS_ID: wr.get('pid'),
                UserLoggedIn.FIELD_SESSION_COMMENT: wr.get('comment'),
            }
            sessions.append(UserLoggedIn.from_dict(payload))
