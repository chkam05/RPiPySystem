...
from system_service.models.network.internal.interface_channel import InterfaceChannel
...
from system_service.models.network.internal.interface_scope_id import InterfaceScopeId
...
class NetworkInterfacesResolver:
    ...
    @classmethod
    def _create_interface_info_model(cls) -> InterfaceInfo:
        """Create an InterfaceInfo object with safe defaults."""
        return InterfaceInfo(
            network='',
            flags=[],
            flags_bin=0,
            mtu=cls._DEFAULT_MTU,
    ...
    @classmethod
    def _parse_header_line(cls, info: InterfaceInfo, line: str) -> None:
        """Parse interface header line and fill basic fields."""
        m = cls._RE_HEADER_GROUPS.match(line)
        if not m:
            return
    ...
    @classmethod
    def _parse_ipv4_line(cls, info: InterfaceInfo, line: str) -> None:
        """Parse IPv4 line and set inet, netmask and broadcast."""
        m = cls._RE_IPV4.search(line)
        if not m:
            return
    ...
    @classmethod
    def _parse_ipv6_line(cls, info: InterfaceInfo, line: str) -> None:
        """Parse IPv6 line and choose the most relevant address."""
        m = cls._RE_IPV6.search(line)
        if not m:
            return
    ...
    @classmethod
    def _parse_meta_line(cls, info: InterfaceInfo, line: str) -> None:
        """Parse metadata line (MAC, txqueuelen, device label)."""
        m_mac = cls._RE_MAC.search(line)
        if m_mac:
            info.ether = m_mac.group(1).lower()

        m_txq = cls._RE_TXQLEN.search(line)
        if m_txq:
            info.txqueuelen = int(m_txq.group(1))

        m_dev = cls._RE_DEV_LABEL.search(line)
        if m_dev:
    ...
    @classmethod
    def _parse_rx_packets_line(cls, stats: InterfaceStatistics, line: str) -> None:
        """Parse RX packets/bytes line into statistics."""
        m = cls._RE_RX_PKTS.search(line)
        if not m:
            return
    ...
    @classmethod
    def _parse_tx_packets_line(cls, stats: InterfaceStatistics, line: str) -> None:
        """Parse TX packets/bytes line into statistics."""
        m = cls._RE_TX_PKTS.search(line)
        if not m:
            return
    ...
    @classmethod
    def _run_ifconfig_command(cls) -> str:
        """
