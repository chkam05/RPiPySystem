...
class UserLoggedIn:
    ...
    @classmethod
    def from_dict(cls, d: Dict[str, Any]) -> 'UserLoggedIn':
        return cls(
    ...
    @classmethod
    def list_from_dicts(cls, rows: List[Dict[str, Any]]) -> List['UserLoggedIn']:
        return [cls.from_dict(r) for r in (rows or [])]

    @staticmethod
    def list_to_dicts(items: List['UserLoggedIn']) -> List[Dict[str, Any]]:
        return [i.to_public() for i in (items or [])]

    @classmethod
    def list_to_public(cls, items: List['UserLoggedIn']) -> List[Dict[str, Any]]:
